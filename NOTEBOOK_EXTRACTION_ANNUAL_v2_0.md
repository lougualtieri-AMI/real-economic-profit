...
═══════════════════════════════════════════
Your reply MUST have two parts per filing, in this exact order:
  (1) SCRATCHPAD — your sourcing and reasoning (Section 1 below), shown step by step.
  (2) FINAL JSON — the structured output (Section 2 below).

Begin your reply with the scratchpad. Do NOT output the JSON by itself. Producing JSON without a preceding scratchpad means the task is NOT complete — redo it with the scratchpad. The scratchpad is where you show your sourcing (which statement/footnote each number came from) and any math, e.g. the SBC grant-date-fair-value sum. It is required even when the answer seems obvious; the sourcing is exactly what gets checked downstream. (For multiple filings, repeat scratchpad-then-JSON per filing, per the batch protocol below.)
...
═══════════════════════════════════════════
(Required per the Output Contract above — show this in full, before the JSON. Do not skip it.)
