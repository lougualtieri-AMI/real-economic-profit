...
Your reply MUST have two parts, in this exact order:
  (1) SCRATCHPAD — your sourcing and the full TTM math (Section 1 below), shown step by step.
  (2) FINAL JSON — the structured output (Section 2 below).

Begin your reply with the scratchpad. Do NOT output the JSON by itself. Producing JSON without a preceding scratchpad means the task is NOT complete — redo it with the scratchpad. The scratchpad is where you show the TTM arithmetic for every flow item (Current YTD + prior-year Annual − prior-year same-quarter YTD = TTM) and cite where each number came from. It is required even when the answer seems obvious; the arithmetic is exactly what gets checked downstream.
